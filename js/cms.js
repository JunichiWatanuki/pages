...
var CMS = {
	...
	settings: {
		...
		siteUrl: '',
		siteNavItems: [
			{ name: 'Github', href: '#', newWindow: true},
			{ name: 'About' }
		],
		pagination: 3,
		postsFolder: './posts',
		...
		footerText: '&copy; ' + new Date().getFullYear() + ' Copyright Junichi Watanuki All Rights Reserved',
		parseSeperator: '---',
	...
	renderPost: function(id) {
		CMS.posts.forEach(function(post){
			if(post.id == id) {
				...
				$tpl.find('.post-date').html(post.date.getFullYear() + '年' + (post.date.getMonth() + 1) + '月' + post.date.getDate() + '日');
	...
	renderPosts: function() {
		CMS.posts.forEach(function(post){
			...
			var title = '<a href="#">' + post.title + '</a>',
				date = post.date.getFullYear() + '年' + (post.date.getMonth() + 1) + '月' + post.date.getDate() + '日',
